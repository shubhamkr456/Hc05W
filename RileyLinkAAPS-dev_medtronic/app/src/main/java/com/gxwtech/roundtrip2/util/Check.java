...
public class Check {
    ...
    public static String isNewTempBasalSafe(TempBasal tempBasal) {
        ...
        if (isTreatmentTooOld(tempBasal.getStart_time()))
            reply += "Treatment is older than " + RT2Const.safety.TREATMENT_MAX_AGE + " mins, rejected.";
        //is TBR supported?
    ...
    public static String isCancelTempBasalSafe(TempBasal tempBasal, Integration integrationAPS, Realm realm) {
        ...
        if (isTreatmentTooOld(tempBasal.getStart_time()))
            reply += "Treatment is older than " + RT2Const.safety.TREATMENT_MAX_AGE + " mins, rejected. ";
        if (!isThisBasalLastActioned(integrationAPS, realm))
            reply += "Current Running Temp Basal does not match this Cancel request, Temp Basal has not been canceled. ";
        //is TBR supported?
        // TODO: 21/02/2016 perform checks here, return empty string for OK or text detailing the issue
    ...
    public static String isBolusSafeToAction(Bolus bolus) {
        ...
        if (isTreatmentTooOld(bolus.getTimestamp()))
            reply += "Treatment is older than " + RT2Const.safety.TREATMENT_MAX_AGE + " mins, rejected. ";
    ...
    private static boolean isThisBasalLastActioned(Integration integrationAPS, Realm realm) {
        ...
        if (lastActive == null)
            return false;                                                       //No basal has ever been active
